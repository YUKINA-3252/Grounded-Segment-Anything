...
def calculate_cluster_features(cluster):
    # mean_color = np.mean(cluster, axis=(0, 1))
    ...
    # return np.hstack([mean_color, grad_mean])
    ...
    return np.array([mean_color[0], mean_color[1], mean_color[2]])
...
# def detect_pattern_change(roi_image, H, W, shreshold=200, step_width=20, step_height=20):
#     row_num = H // step_height
#     colomn_num = W // step_width
#     features = np.zeros((row_num, colomn_num, 3))
#     for i in range(row_num):
#         for j in range(colomn_num):
#             cluster = roi_image[step_height*i:step_height*(i+1), step_width*j:step_width*(j+1)]
#             features[i, j] = calculate_cluster_features(cluster)
#     # for start_x in range(0, roi_image.shape[1] - step_width, step_width):
#     #     end_x = start_x + step_width
#     #     for start_y in range(0, roi_image.shape[0] - step_height, step_height):
#     #         end_y = start_y + step_height
#     #         region = roi_image[start_y:end_y, start_x:end_x]
#     #         clusters.append((start_x, start_y, region, features))
#     # for i in range(len(clusters) - 1):
#     #     features.append(calculate_cluster_features(clusters[i][2]))
#     # for i in range(len(features) - 1):
#     #     distance = np.linalg.norm(features[i] - features[i + 1])
#     #     if distance > threshold:
#     #         changes.append(clusters[i])

#     # return changes, clusters, (x, y, w, h)
#     features = normalize_features(features)
#     features_diff = np.zeros((row_num,colomn_num-1))
#     for i in range(row_num):
#         for j in range(colomn_num-1):
#             diff = np.linalg.norm(features[i, j] - features[i, j+1])
#             features_diff[i, j] = diff
#     return features, features_diff

def detect_pattern_change(image, X, Y, H, W):
    # roi_image = image[y:y+h, x:x+w]
    # print(COLOR_H)
    # print(len(COLOR_H))
    # print(len(COLOR_H[0]))
    # print(X, Y)
    # print(COLOR_H[71][348])
    # print(COLOR_H[71][349])
    rgb_diff = np.zeros((H, W-1))
    for i in range(H):
        for j in range(W-1):
            # diff = abs(COLOR_H[Y+i][X+j] - COLOR_H[Y+i][X+j+1])
            # diff = np.minimum(diff, 360 - diff)
            # h_diff[i, j] = diff
            diff = np.linalg.norm(image[Y+i][X+j] - image[Y+i][X+j+1])
            rgb_diff[i,j] = diff
    print(rgb_diff)
    return rgb_diff
...
# for start_x, start_y, _, features in clusters:
#     cv2.rectangle(image, (x + start_x, y + start_y), (x + start_x + step_width, y + start_y + step_height), (0, 0, 255), 2)
#     cv2.putText(image, str(features), (x + start_x + int(step_width / 2), y + start_y + int(step_height / 2)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 1, cv2.LINE_AA)
...
def filter_frequencies(fshift, filter_type='low', cutoff=30):
...
def deskew_image(image):
    ...
    a4_width = 2480  # 210mm at 300dpi
    a4_height = 3508  # 297mm at 300dpi

    src_pts = np.float32(sorted(box, key=lambda x: (x[1], x[0])))
    if src_pts[1][0] > src_pts[2][0]:
        src_pts[[1, 2]] = src_pts[[2, 1]]

    dst_pts = np.float32([[0, 0], [a4_width, 0], [a4_width, a4_height], [0, a4_height]])

    matrix = cv2.getPerspectiveTransform(src_pts, dst_pts)

    aligned = cv2.warpPerspective(image, matrix, (a4_width, a4_height))
    return aligned


if __name__ == "__main__":
    directory_path = "ros/fold/pattern/rgb"
    latest_file_path = os.path.join(directory_path, 'top_paper_1.jpg')
    ...
    # result_image = detect_discontinuity_in_region(image, sum_paper_mask, patch_size=20, threshold=0.3)
    # cv2.imwrite("ros/fold/detect_discontinuity.jpg", result_image)

    # # detect edge
    # edges = detect_edges_in_region(image, sum_paper_mask, low_threshold=50, high_threshold=150)
    # result_image = image.copy()
    # result_image[edges>0] = (0, 0, 255)
    # cv2.imwrite("ros/fold/edges_image.jpg", result_image)
    ...
    # depth_image_directory_path = "ros/fold/pattern/depth/top_paper.png"
    ...
    # height = int(pts_base_coords[np.argmax(pts_base_coords[:, 0])][0] - pts_3d[np.argmin(pts_base_coords[:, 0])][0])
    ...
    # # dst_pts = np.array([[[0, 0]], [[200, 0]], [[0, 600]], [[200, 600]]], dtype="int32")
    ...
    # # color_h, color_s, color_v = cv2.split(hsv)
    # # roi_image = cv2.bitwise_and(image, image, mask=sum_paper_mask)
    # # roi_image = cv2.cvtColor(roi_image, cv2.COLOR_BGR2RGB)
    # # cv2.imwrite("ros/fold/roi.jpg", roi_image)
    # # rotated_image = deskew_image(roi_image)
    # result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
    # cv2.imwrite("ros/fold/pattern/result1.jpg", result)

    # # width = 10
    # # height = 20
    # # rgb_diff = detect_pattern_change(image, x, y, h, w)
    # # # highlighted_image = highlight_changes(image.copy, features, h, w, step_width=width, step_height=height)
    # # highlighted_image = highlight_changes(sum_paper_mask, rgb_diff, x, y, h, w)
    # # cv2.imwrite('ros/fold/color_change.jpg', highlighted_image)

    # # # detect pattern changes
    # # num_slices = 10
    # # threshold = 50
    # # distances, change_regions = detect_horizontal_changes(image, sum_paper_mask, num_slices, threshold)
    # # result_image = image.copy()
    # # for (x_start, x_end) in change_regions:
    # #     cv2.rectangle(result_image, (x_start, 0), (x_end, image.shape[0]), color=(0, 0, 255), thickness=2)
    # # cv2.imwrite("ros/fold/detect_pattern_changes.jpg", result_image)
